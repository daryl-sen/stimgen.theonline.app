...
cobloAdj = Blueprint('cobloAdj', __name__, template_folder = 'templates/cobloAdj')
...
def run(ref_id, run_mode, target_mode):
  def generate_target_url(run_mode, target_mode, target):
    ...
    return url_for('cobloAdj.run', ref_id=ref_id, run_mode=new_run_mode, target_mode=new_target_mode)
...
def projects(ref_id):
  ...
  if form.validate_on_submit():
    ...
    if mode == 'edit':
      ...
      return redirect(url_for('cobloAdj.projects', ref_id=target_project.ref_id))
    elif mode == 'create':
      ...
      return redirect(url_for('cobloAdj.projects', ref_id=new_project.ref_id))
  else:
